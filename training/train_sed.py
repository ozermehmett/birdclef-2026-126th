#!/usr/bin/env python3
"""Distilled SED Training Script for BirdCLEF+ 2026.

EfficientNet-B0 Sound Event Detection training script for BirdCLEF+ 2026 (135th place).
...
Usage (single fold):
    python train_sed.py --fold 0 --gpu 0

Usage (all folds, background):
    GPU=0 nohup bash scripts/auto_sed.sh > outputs/logs/sed_train.log 2>&1 &
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
# ── Paths (Colab A100 + local-scratch NVMe + Drive backup) ──────
ROOT         = Path(os.environ.get("BIRDCLEF_ROOT", "/mnt/local-scratch"))
COMP_DIR     = Path(os.environ.get("BIRDCLEF_COMP", ROOT / "birdclef-2026"))
CACHE_DIR    = Path(os.environ.get("BIRDCLEF_CACHE", ROOT / "waveform_cache"))
PERCH_ONNX   = Path(os.environ.get("PERCH_ONNX_PATH",
                                    ROOT / "perch-v2-no-dft-onnx/perch_v2_no_dft.onnx"))
OUT_ROOT     = Path(os.environ.get("BIRDCLEF_OUT", ROOT / "outputs/sed"))
DRIVE_BACKUP = Path(os.environ.get("DRIVE_BACKUP_DIR",
                                    "/content/drive/MyDrive/birdclef/outputs"))
...
# ══════════════════════════════════════════════════════════════════════════════
# Mel + SpecAugment (GPU, torchaudio)
# ══════════════════════════════════════════════════════════════════════════════
import torchaudio
...
class MelSpecTransform(nn.Module):
    ...
    GPU Mel Spectrogram transform for SED training."""
    def __init__(self):
        super().__init__()
...
def extract_chunk(waveform, start_sample, n_samples):
    """Extract chunk with LEFT-padding if recording is too short."""
    total = len(waveform)
    if total <= n_samples:
...
class FocalDS(Dataset):
    """Focal recording dataset. Returns 5-tuple (waveform, label, weight, mask, source_tag)."""
...
def export_onnx(trained_state, backbone_dim, out_path, device, backbone_name=None):
    ...
    if diff >= 5e-2:
        print(f"    WARNING: ONNX export diff={diff:.4f} > 5e-2 threshold")
        print(f"    Model still saved but verify quality with separate test")
    elif diff >= 1e-2:
        print(f"    NOTE: ONNX export diff={diff:.4f} (acceptable, FP16 rounding)")
    # Crash etme
    del exp, sess
...
def train_fold(fold_k, audio_cache_meta, sc_cache_meta, Y_SC, non_s22_mask_sc,
    ...
    for ep in range(EPOCHS):
        ...
        for wav, lb, wt, mk, sr in tl:
            ...
            with autocast():
                ...
                # Taxon-weighted loss: upweight Insecta (2.5x) and Amphibia (2.0x)
                # These taxa are underrepresented vs dominant Aves in focal recordings.
                # Weights applied per-class before batch reduction so Aves gradients
                # are not suppressed — only rare taxa get a boost.
                if LOSS_TYPE == "asl":
                ...
                # Apply taxon loss weights: Insecta 2.5x, Amphibia 2.0x
                # Built as a class-level multiplier on raw [B, C] loss tensor.
                # Values chosen conservatively: high enough to boost rare taxa,
                # low enough to not destabilise Aves (majority class) training.
                taxon_w = torch.ones(NUM_CLASSES, device=device, dtype=raw.dtype)
                for _idx in taxon_masks.get("Insecta", np.array([])):
                    taxon_w[_idx] = 2.5
                for _idx in taxon_masks.get("Amphibia", np.array([])):
                    taxon_w[_idx] = 2.0
                raw = raw * taxon_w.unsqueeze(0)  # broadcast: [B, C] * [1, C]

                ps = (raw * wt * mk).sum(1) / (mk.sum(1) + 1e-8)
                cls_loss = (ps * sw).mean()

                # Distillation: same waveform fed to Perch as mel sees
    ...
    # Save both ns22 and macro checkpoints + ONNX (both may differ significantly on LB)
    ...
    # Default ONNX alias: ns22 preferred (test both on LB)
...
# ══════════════════════════════════════════════════════════════════════════════
# Data loading + fold assignment
# ══════════════════════════════════════════════════════════════════════════════


def backup_fold_to_drive(out_dir, fold_k):
    """Her fold biter bitmez Drive'a kopya. Session olunce kayip yok."""
    import shutil, os
    try:
        # Use mkdir directly — parent.exists() can return False during Drive mount
        # delays, causing backup to be skipped incorrectly.
        target = DRIVE_BACKUP / out_dir.name
        target.mkdir(parents=True, exist_ok=True)
        copied = 0
        for f in out_dir.glob(f"fold{fold_k}_*"):
            shutil.copy2(str(f), str(target / f.name))
            copied += 1
        for f in out_dir.glob(f"sed_fold{fold_k}*"):
            shutil.copy2(str(f), str(target / f.name))
            copied += 1
        print(f"  Drive backup: {copied} files -> {target}")
    except Exception as e:
        print(f"  Drive backup FAILED (training devam): {e}")
...
def main():
    global EPOCHS, BATCH, BACKBONE_NAME, DROP_PATH_RATE, ALPHA_DISTILL, \
           ...
           PSEUDO_UNLABELED_SC_CSV, UNLABELED_SC_CACHE_DIR, SHARES
    ap = argparse.ArgumentParser()
    ap.add_argument("--fold",           type=int,   default=0)
    ap.add_argument("--gpu",            type=int,   default=0)
    ap.add_argument("--out",            type=str,   default=None,
                    help="output dir (default: outputs/sed-<backbone>)")
    ...
    ap.add_argument("--sc_share",        type=float, default=None,
                    help="Labeled SC batch share (default 0.1). Recommended 0.15 for clean train.")
    args = ap.parse_args()
    ...
    if args.sc_share is not None:
        sc = float(args.sc_share)
        SHARES["sc"] = sc
        SHARES["focal"] = 1.0 - sc
        print(f"  SC share override: focal={SHARES['focal']:.2f}  sc={SHARES['sc']:.2f}")

    short = _BACKBONE_SHORT.get(BACKBONE_NAME, BACKBONE_NAME.split(".")[0])
    out_default = ROOT / f"outputs/sed-{short}"
    ...
    assert CACHE_DIR.exists(), f"Waveform cache not found: {CACHE_DIR}\n" \
        "Download: kaggle datasets download tuckerarrants/birdclef-2026-waveform-cache  # or build with build_waveform_cache.py"
    ...
    for fold_k in folds_to_run:
        ...
        backup_fold_to_drive(out_dir, fold_k)
